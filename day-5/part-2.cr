  ...
  update[..-2].each_with_index do |page, idx|
    update[idx+1..].each do |following_page|
      if !ordering_rules.has_key?(page) || !ordering_rules[page].includes?(following_page)
        return fix_page_order(update, ordering_rules)
      end
    end
  end
  return 0
...
def fix_page_order(update, ordering_rules)
  sorted = update.sort do |a, b|
    if ordering_rules.has_key?(a) && ordering_rules[a].includes?(b)
      -1
    else
      1
    end
  end
  mid_idx = (sorted.size / 2).floor.to_i32
  middle_page = sorted[mid_idx].to_i8
  return middle_page
end